...
event * pf_io_done_notify (ioreq_event *curr, void *ctx)
{
   ...
   if (pf_printhack) {
     fprintf (outputfile, "pf_io_done_notify: curr->buf %p, curr->opid %ld, curr->blkno %ld\n", curr->buf, curr->opid, curr->blkno);
   }
...
{
   if (pf_printhack)
   fprintf (outputfile, "%f\tIOACC cpu=%d opid=%ld blkno=%ld\n", simtime, cpu_ev->cpunum, curr->opid, curr->blkno);
...
{
   if (pf_printhack)
   fprintf (outputfile, "%f\tIO INTERNAL cpu=%d type %d opid=%ld blkno=%ld buf=%p\n", simtime, cpu_ev->cpunum, curr->type, curr->opid, curr->blkno, curr->buf);
