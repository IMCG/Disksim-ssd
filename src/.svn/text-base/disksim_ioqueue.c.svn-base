			     ...
			     int cylmaptype)
{
   ...
   switch (cylmaptype) {
	 ...
	 * it can't really hurt us...  */
         ...
         *cylptr = (int)(long) curr->iolist->buf;
			ASSERT((void*)(long)(*cylptr) == curr->iolist->buf);
	 *surfptr = 0;
         break;
...
/* bcount undefined for completing requests */
   ...
   if (curr->batch_list != NULL) {
     ...
     if ((curr->blkno <= wanted->blkno) && ((curr->blkno + curr->totalsize) >= wanted->blkno)) {
      if ((curr->blkno == wanted->blkno) && (curr->opid == wanted->opid) 
				         && ((curr->flags & READ) == (wanted->flags & READ))) {
	 return(1);
       }
       tmp = curr->iolist;
       while (tmp) {
...
*/
   ...
   if ((tmp->state != PENDING) || (ioqueue_request_match(done, tmp) == 0)) {
	 ...
	 tmp = tmp->next;
      ...
      if ((tmp->state != PENDING) || (ioqueue_request_match(done, tmp) == 0)) {
	 fprintf(stderr, "Completed event not found pending in ioqueue - blkno %ld, tmp->blkno %d, tmp->totalsize %d, %d, %ld, %ld, %d\n", done->blkno, tmp->blkno, tmp->totalsize, (tmp == NULL), done->opid, tmp->opid, done->bcount);
...
static int ioqueue_raise_priority_subqueue (subqueue *queue, subqueue *priority, long int opid, int pri_scheme)
{
   int stop = FALSE;
   iobuf *tmp;
   ioreq_event *trv;
...
int ioqueue_raise_priority (ioqueue *queue, long int opid)
{
   int found = 0;
