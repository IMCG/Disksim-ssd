...
*/
      ...
      default:
	 fprintf(stderr, "Invalid hittype in disk_buffer_stats - blkno %ld, bcount %d, state %d\n", curr->blkno, curr->bcount, seg->state);
         exit(1);
   }
   return;
}
...
fflush(outputfile);
   ...
   switch (seg->state) {
	 ...
	 break;
      ...
      default:
	 fprintf(stderr, "Invalid segment state in disk_buffer_set_segment - blkno %ld, bcount %d, state %d\n", currdiskreq->ioreqlist->blkno, currdiskreq->ioreqlist->bcount, seg->state);
         exit(1);
   }
