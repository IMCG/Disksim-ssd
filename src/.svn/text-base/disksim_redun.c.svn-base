...
*/
         if (distance < 0) {
            fprintf(stderr, "Integrity check failure at logorg_join_seqreqs - blkno %ld, bcount %d, blkno %ld, read %d\n", temp->blkno, temp->bcount, temp->next->blkno, (temp->flags & READ));
            exit(1);
         }
...
static void logorg_parity_table_read_old (logorg *currlogorg, ioreq_event *rowhead, ioreq_event **reqlist, long int opid)
{
   ioreq_event *temp;
   ioreq_event *newreq;
...
static void logorg_parity_table_recon (logorg *currlogorg, ioreq_event *rowhead, ioreq_event **reqlist, int stripeno, int unitno, int tableadd, long int opid)
{
   ioreq_event *temp;
   ioreq_event *newreq;
   int entryno;
