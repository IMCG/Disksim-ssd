...
#define MAX_QUEUE_LENGTH 2000000000
...
//    int minutes=5;
//   int count_1000 = 0;
//   if(currlogorg->stat.outstanding>1000) 
//     count_1000 = 1;
//   else if (currlogorg->stat.outstanding<100)
//    prev_count = 1; 
//   else if (currlogorg->stat.outstanding>2000)
//    prev_count = 1;
   
/*   if( (simtime<1018128 && simtime>987456) || (simtime<67002467 && simtime>66971794.62) )
    minutes=0.5;
   else
    minutes=5;
   if(simtime/(minutes*60000) >prev_count)
   {
      fprintf(stderr,"OUT:%d,MAX:%d @ %f\n",currlogorg->stat.outstanding,MAX_QUEUE_LENGTH,simtime);
      prev_count++;
   }
*/
  ...
  int freq =10000000; 
  num_request++;
  if( num_request > 68040000)
  {
    freq = 1;
  }
  else if( num_request > 68030000)
  {
    freq = 10;
  }
  if( num_request > 69000000)
  {
    freq = 100000;
  }
  if(num_request % freq == 0 && debug_print == 1)
  {
    printf("%d requests have been queued @%f. Outstanding requests:%d\n",num_request,simtime,currlogorg->stat.outstanding);
  }
#endif
   if (currlogorg->stat.outstanding >= MAX_QUEUE_LENGTH) {
      fprintf(stderr,"Stopping simulation because of saturation: simtime %f, totalreqs,  %d queue length:%d\n", simtime, disksim->totalreqs,currlogorg->stat.outstanding);
      fflush(stderr);
      fprintf (outputfile, "Stopping simulation because of saturation: simtime %f, totalreqs %d\n", simtime, disksim->totalreqs);
      fprintf (outputfile, "last request:  dev=%d, blk=%ld, cnt=%d, %d (R==1)\n",curr->devno, curr->blkno, curr->bcount, (curr->flags & READ));
      disksim_simstop();
   }
   if (req->flags & READ) {
...
static void logorg_mapcomplete_update_stats (logorg *currlogorg, ioreq_event *curr, outstand *req)
{
   ...
   if(num_request % freq == 0 && debug_print == 1)
   {
     printf("%d requests have been de-queued @%f. Outstanding requests:%d\n",num_request,simtime,currlogorg->stat.outstanding);
   }
#endif
   if (req->flags & READ) {
      currlogorg->stat.readoutstanding--;
...
static outstand * logorg_getfromoutstandq (logorg *currlogorg, long int requestno)
{
   outstand *temp = NULL;
   outstand *hold = NULL;
   int i;
...
*/
  ...
  if (debug_print) {
    count++;
    fprintf(stderr,"%d %d\n",logorgs[0]->outstandqlen,logorgs[0]->stat.outstanding);
  }
#endif
   ...
   if (logorgno == -1) {
      fprintf (stderr, "unexpected request location: devno %x, blkno %ld, bcount %d\n", curr->devno, curr->blkno, curr->bcount);
   }
   ASSERT(logorgno != -1);
...
void logorg_raise_priority (logorg **logorgs, int numlogorgs, long int opid, int devno, int blkno, void *buf)
{
   int i, j;
   int logorgno = -1;
   int calc;
