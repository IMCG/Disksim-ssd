...
void disksim_simulate_event (int num)
{
  ...
  if ((curr = getnextevent()) == NULL){
    if ((curr = getfromintq()) == NULL){
    disksim_simstop ();
    return;
    }
  } 
  temp = (ioreq_event*)curr;
...
static void disksim_setup_outputfile (char *filename, char *mode)
{
   ...
   fprintf(stderr,"Setting up output file:%s,mode:%s\n",filename,mode);
...
{
	FILE *temp = 0;
	char *name = (char*)malloc(sizeof(char)*201);
	char *ret = 0;
	assert(master_trace_file);
	ret = fgets(name,200,master_trace_file);
	if(ret)
	{
		sscanf(name,"%s\n",name);
		assert(name);
		...
		if(!temp)
		{
			fprintf(stderr,"FATAL:Unable to open trace file\n");
			exit(1);
		}
	}
	if(name)
...
static void disksim_setup_iotracefile (char *filename)
{
   if (strcmp(filename, "0") != 0) {
	 ...
	 disksim->iotracefile = gzdopen(0, "rb");
      } else {
	//master_trace_file = 0;
	...
	//disksim->iotracefile = open_next_file();
	//preprocess_tracefile(disksim->iotracefile);
      }
   } else {
...
void preprocess_tracefile(FILE *tracefile)
{
	rewind(tracefile);
	char line[200]; int i;
	ioreq_event currequest;
	disksim->no_of_devices_in_trace = -1;
	for(i=0;i<100;i++)
		disksim->max_block_per_device_in_trace[i] = -1;
	while(fgets(line,199,tracefile))
	{
		sscanf(line, "%lf %d %ld %d %x\n", &currequest.time,&currequest.devno,&currequest.blkno,
											&currequest.bcount,&currequest.flags);
		if(disksim->max_block_per_device_in_trace[currequest.devno]<currequest.blkno)
		 	disksim->max_block_per_device_in_trace[currequest.devno] = currequest.blkno;
		if(disksim->no_of_devices_in_trace<currequest.devno)
			disksim->no_of_devices_in_trace = currequest.devno;
	}
	disksim->no_of_devices_in_trace++;//assuming device nos in trace start from 0.
	rewind(tracefile);
}
...
// CHANGED: changed the way to input the params
...
/*
  if (argc < 9) {
    fprintf(stderr,"Usage: %s paramfile outfile format iotrace synthgen? repeat_trace_count? snapshot_output_path? ds_snapshot(0)/ta_snapshot(1)/none(-1)? base_snapshot_index_path?\n", argv[0]);
    exit(1);
  }
*/

/* Usage: disksim paramfile outfile format iotrace synthgen? total_trace_repeat_times snapshot_output_dir simulation_strategy(detailed? single_threshold?)  threshold? detailed_simu_duration? fast_forward_strategy? fast_forward_duration? */
  if(argc < 9)
  {
	fprintf(stderr, "Usage: %s paramfile outfile format iotrace synthgen? repeat_trace_count? simulation_strategy? snapshot_output_path?\n", argv[0]);
	exit(1);
  }
  ...
  fprintf (stderr, "%s %s %s %s %s %s\n", argv[0], argv[1], argv[2], argv[3],
	   argv[4], argv[5]);
#endif
// don't need to separate snapshots into da_snapshot and ta_snapshot anymore; all the snapshots are put into one dir
// int is_ta_snapshot = atoi(argv[8]); 

// not deal with parameter override for now;
/*
  if (is_ta_snapshot == -1) {
    if ((argc - 9) % 3 > 1) {
  ...
  else if(is_ta_snapshot == 0 || is_ta_snapshot == 1) {
    if (argc>=9 && (argc - 10) % 3 > 1) {
      fprintf(stderr, "Parameter file overrides must be 3-tuples\n");
      exit(1);
    }
  }
...
//  disksim->snapshot_startindex=1;
  // KJ: some initialization done here
  disksim->snapshot_currindex = 1;
  disksim->cur_detailed_trace_count = 0;
  disksim->is_past_threshold = 0;
  disksim->cur_repeat = 0;
  disksim->duration = 0;


  outputfile = 0;
  snapshotfile = 0;
  // KJ: keep the following 2 to be 0 
  // previous version reads stress information from ds/ta snapshots to initialize if the following 2 are none NULL
  ds_snapshotfile = 0;
  ta_snapshotfile = 0;
  
// to get the base snapshot from fast_forward mode
// KJ: should not be read from here
/*
  if(is_ta_snapshot != -1) {
    strcpy(ip_snapshotfilename,argv[9]);
    assert(argc>9);

    if(is_ta_snapshot)
      ta_snapshotfile = fopen(argv[9],"r");    
    else
      ds_snapshotfile = fopen(argv[9],"r");
	  
	char* ds_substr= strtok(argv[9],"/");
	char *prev_ds = 0;
	while(ds_substr!=0) {
	  prev_ds = ds_substr;
	  ds_substr = strtok(NULL,"/");
	}
	sscanf(prev_ds,"snapshot%d.outv",&disksim->snapshot_startindex);
	disksim->snapshot_startindex++;
	fprintf(stderr,"Start index for snapshot is %d\n",disksim->snapshot_startindex);
  }
*/
  char *base_filename = argv[2];
  char *output_file_name = (char*)malloc(sizeof(char)*500);

  strcpy(disksim->output_file_prefix,base_filename);
  disksim->output_file_prefix[strlen(disksim->output_file_prefix)-5] = 0;//assumes the extension for output file is 5 characters long, which is .outv
  sprintf(output_file_name,"%s_snapshot%d.outv", disksim->output_file_prefix,disksim->snapshot_currindex);

  // KJ: dump previous trace info in prev_trace_file;
 // char *prev_trace_file_name = (char*)malloc(sizeof(char)*500);
 // sprintf(disksim->prev_trace_file_name, "%s_prev_trace.outv", disksim->output_file_prefix);
  disksim->curr_record = (struct prev_trace_record *) malloc (sizeof(struct prev_trace_record));
  assert(disksim->curr_record);
  disksim->curr_record->index = 0;
  disksim->curr_record->curr_len = 1000;
  disksim->curr_record->head = (struct prev_trace_entry *) malloc (disksim->curr_record->curr_len * sizeof(struct prev_trace_entry));
  ...
  fprintf (outputfile, "\n*** Output file name: %s\n", argv[2]); 
  fprintf(outputfile,"Start index for snapshot is %d\n",disksim->snapshot_currindex);    
  free(output_file_name);
  fflush (outputfile); 

  // initialize trace format
  ...
  // This is the total repeat trace times of the simulation
  if(argv[6]){
		disksim->repeat_trace = atoi(argv[6]);
	}
	
  // This is the output dir of the snapshot files;
  if(argv[7]) {
  	ddbg_assert(strlen(argv[7])<400);
    strcpy(disksim->snapshot_output_path,argv[7]);
  	fprintf(outputfile,"Using %s as the output path for snapshotfile files\n",disksim->snapshot_output_path);
  	}
  	else {
  		strcpy(disksim->snapshot_output_path,"/localtmp");
  		fprintf(outputfile,"WARNING.. Using default path /localtmp/ as the output path for block-level info\n");
	}
  disksim->curr_snapshot_output_file = 0;

  fprintf(outputfile, "Repeat trace count:%d\n",disksim->repeat_trace);
  fprintf(outputfile, "*** Synthgen to be used?: %d\n\n", disksim->synthgen); 
  fflush(outputfile); 

  // KJ: handle more args here
  // change disksim struct in disksim_global.h accordingly
  disksim->simulation_strategy = atoi(argv[8]);
  // 0 for all-detailed simulation; 1 for single-threshold simulation;
  fprintf(outputfile, "Simulation mode to be used?: %d\n", disksim->simulation_strategy);
  fflush(outputfile);

  // single-threshold mode
  if(disksim->simulation_strategy == 1)
  {
//	fprintf(stderr, "######");
	disksim->threshold = atof(argv[9]);
	fprintf(stderr, "Threshold is %lf\n", disksim->threshold);
	disksim->detailed_simulation_duration = atoi(argv[10]);
	// for now, only has 1 fast_forward strategy;
	disksim->fast_forward_strategy = atoi(argv[11]);
	disksim->fast_forward_duration = atoi(argv[12]);
  }
  
/*
  if (is_ta_snapshot == -1) {
    disksim->overrides = argv + 9;
    disksim->overrides_len = argc - 9;
  }
  else {
    disksim->overrides = argv + 10;
    disksim->overrides_len = argc - 10;
  }
*/
 ...
 // TODO: didn't take para overriding into consideration;
  disksim->overrides = argv + argc;
  disksim->overrides_len = 0;
...
/*    } */
  fprintf(stderr, "begin loading params\n");
  ...
  fprintf(stderr, "done with loading params\n");
  ...
  fflush(outputfile);
  // This function handles the 1st request;
  prime_simulation();
...
{
  // KJ: the modified version dumps all the snapshots(including those generated from detailed and fast-forward to one dir)
  // use snapshot-currindex to index all the snapshots
  if(!disksim->curr_snapshot_output_file) //free(disksim->curr_snapshot_output_file);
    disksim->curr_snapshot_output_file = (char*)malloc((strlen(disksim->snapshot_output_path)+150)*sizeof(char));

  assert(disksim->curr_snapshot_output_file);
//  sprintf(disksim->curr_snapshot_output_file,"%ssnapshot%d.outv",disksim->snapshot_output_path,disksim->snapshot_startindex);
  sprintf(disksim->curr_snapshot_output_file,"%ssnapshot%d.outv",disksim->snapshot_output_path,disksim->snapshot_currindex);
  // snapshotfile is disksim->snapshotfile
  snapshotfile = fopen(disksim->curr_snapshot_output_file,"w");
  fprintf(stderr,"Current snapshot file:%s\n",disksim->curr_snapshot_output_file);
  assert(snapshotfile);

//  disksim->snapshot_startindex++;
  disksim->snapshot_currindex++;
  
  // KJ: TODO: why it seems that outputfile is changed to next one before stats are dumped?
...
// KJ: set prev_trace_file here
void set_prev_trace_file(struct prev_trace_record *curr_record)
{

	assert(curr_record);		
	curr_record->index = 0;
	// intialize all the array to 0
    	memset(curr_record->head, 0, sizeof(struct prev_trace_entry) * curr_record->curr_len);	
	
/*
	fprintf(stderr, "Setting up prev_trace file: %s, mode %s\n", filename, mode);
	if((disksim->prev_trace_file = fopen(filename, mode)) == NULL)
	{
		fprintf(stderr, "Prev_trace file %s cannot be opened for write access\n", filename);
		exit(1);
	}
*/
/*	
	if(strlen(filename) < 499)
	{
		strcpy(disksim->prev_trace_file_name, filename);
	}else{
		fprintf(stderr, "Filename %s for prev_trace_file is too long\n", filename);
		exit(1);
	}
*/
	// not sure the way to use the following function
//	setvbuf(disksim->prev_trace_file, 0, _IONBF, 0);
...
void movefile(char *src,char *dst)
{
  ...
  fstat(fd1,&st1);
  int size = st1.st_size;//size is stored in st.st_size.

  int chunks = 1024*1024;//transfer in 1MB chunks.
  char *buffer = (char*)malloc(sizeof(char)*chunks);
  FILE* src_fp = fdopen(fd1,"rb");
  FILE* dst_fp = fopen(dst,"wb");
  while(size-chunks>0)
  {
    size-=chunks;
    assert(fread((void*)buffer,sizeof(char),chunks,src_fp) == chunks*sizeof(char));
    assert(fwrite((void*)buffer,sizeof(char),chunks,dst_fp)== chunks*sizeof(char));
  }
  assert(fread((void*)buffer,sizeof(char),size,src_fp) == size*sizeof(char));
  assert(fwrite((void*)buffer,sizeof(char),size,dst_fp)== size*sizeof(char));
  fclose(src_fp);
  fclose(dst_fp);
  free(buffer);
  ...
  fstat(fd2,&st2);
  assert(st1.st_size == st2.st_size);//Make sure both the file sizes are same.
