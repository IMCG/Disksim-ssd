...
/*
fprintf (outputfile, "%f, oversized request: opid %ld, blkno %d, bcount %d, maxreqsize %d\n", simtime, curr->opid, curr->blkno, curr->bcount, ctl->maxreqsize);
*/
...
void iodriver_access_complete (int iodriverno, intr_event *intrp)
{
   ...
   if (iodrivers[iodriverno]->devices[(req->devno)].queuectlr != -1) 
   {
	...
	// fprintf (outputfile, "oversized request in list: opid %ld, blkno %d, bcount %d\n", tmp->opid, tmp->blkno, tmp->bcount);
	...
	tmp = tmp->next;
	  ...
	  && (req->blkno < (tmp->next->blkno + tmp->next->bcount))) 
      {
	fprintf (outputfile, "%f, part of oversized request completed: opid %ld, blkno %ld, bcount %d, maxreqsize %d\n", simtime, req->opid, req->blkno, req->bcount, ctl->maxreqsize);
...
void iodriver_printstats(int sub_status)
{
   int i;
   int j;
   struct ioq **queueset;
