				  ...
				  ioreq_event *curr)
{
  ...
  if(currdisk->const_acctime) {
	...
	disk_interferestats(currdisk, tmpioreq);
      ...
      evtfound = (int)(long)ioqueue_physical_access_done(currdisk->queue,tmpioreq);
...
// 	 curr->blkno);
  ...
  if(disk_printhack && (simtime >= disk_printhacktime)) {
    fprintf (outputfile, "%12.6f            Entering disk_initiate_seek to %ld for disk %d\n", simtime, curr->blkno, curr->devno);
    fflush(outputfile);
  }
