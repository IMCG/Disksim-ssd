...
static char *statdesc_acctimestats  =  (char*)"Access time";
...
{
   disksim->ssdinfo = (ssdinfo_t*) malloc (sizeof(ssdinfo_t));
   bzero ((char *)disksim->ssdinfo, sizeof(ssdinfo_t));
   disksim->ssdinfo->ssds = (ssd_t**) malloc(MAXDEVICES * (sizeof(ssd_t)));
...
{
  //Warning: Make sure any change in traceanalyzer's dumpobject() is updated here.
  assert(ta_snapshotfile);
  char temp1[20],temp2[20];
  char line1[200];
  int el_num = -1, block_num = -1,i=0,j=0,plane_num = -2;
  char dead_or_alive;
  int dead_block_count = 0;
  if(elem_num == 0) {
    fprintf(stderr,"Reading snapshot from %s\n",ip_snapshotfilename);
    //fprintf(stderr,"Warning. Setting stress increment to 40\n");
    //fprintf(stdout,"Warning. Setting stress increment to 40\n");
  }
  for(i=0;i<currdisk->params.blocks_per_element;i++) {
    el_num = -1;
    block_num = -1;
    plane_num = -2;
    block_metadata *currblock = &metadata->block_usage[i];
    assert(fgets(line1,200,ta_snapshotfile));
    assert(sscanf(line1,"\"D #0\" \"E #\" %d \"P #\" %d \"B #\" %d\n",&el_num,&plane_num,&block_num) == 3);
    assert(el_num == elem_num);
    assert(block_num == i);
    if(plane_num!=-1)
      assert(plane_num == currblock->plane_num);
    assert(fgets(line1,200,ta_snapshotfile));
    assert(sscanf(line1,"%c %ld %ld %lf %d %d %d %lf %lf\n",&dead_or_alive,
                                            &currblock->logical_stresses,
                                            &currblock->physical_stresses,
                                            &currblock->least_retention_page->delvth,
                                            &currblock->least_retention_page->logical_stresses,
                                            &currblock->least_retention_page->physical_stresses,
                                            &currblock->least_retention_page->stress_increment,
                                            &currblock->least_retention_page->eqn_cycle,
                                            &currblock->least_retention_page->retention_period) == 9);
    currblock->least_retention_page->retention_period*=YEAR;
     //if(ssd_block_dead(currblock,currdisk)) {
      //dead_block_count++;
    //}
    currblock->logical_stresses+=INITIAL_STRESSES*currdisk->params.pages_per_block;
    //currblock->least_retention_page->logical_stresses+=INITIAL_STRESSES;
    

    //for all the pages in this block, set the least retention page info too. 
    //Hence the loop index 'j' starts with 1.
    for(j=1;j<currdisk->params.pages_per_block;j++) {
      currblock->page[j].delvth = currblock->least_retention_page->delvth;
      currblock->page[j].logical_stresses = currblock->least_retention_page->logical_stresses;
      currblock->page[j].physical_stresses = currblock->least_retention_page->physical_stresses;
      currblock->page[j].stress_increment = currblock->least_retention_page->physical_stresses;
      currblock->page[j].eqn_cycle = currblock->least_retention_page->eqn_cycle;
      currblock->page[j].retention_period = currblock->least_retention_page->retention_period;
    }
  }
  return dead_block_count;
}

int ssd_init_stress_info_ds(ssd_t *currdisk, ssd_element_metadata *metadata,int elem_num)
{
  //Warning: Make sure any change in traceanalyzer's dumpobject() is updated here.
  assert(ds_snapshotfile);
  char temp1[20],temp2[20];
  char line1[200];
  int el_num = -1, block_num = -1,i=0,j=0,plane_num = -2;
  char dead_or_alive;
  int dead_block_count = 0;
  if (elem_num == 0) {
    fprintf(stderr,"Reading ssd reliability status from %s\n",ip_snapshotfilename);
    assert(fgets(line1,200,ds_snapshotfile));
    assert(fgets(line1,200,ds_snapshotfile));
  }
  for(i=0;i<currdisk->params.blocks_per_element;i++) {
    el_num = -1;
    ...
    for(j=1;j<currdisk->params.pages_per_block;j++) {
      ...
      currblock->page[j].physical_stresses = currblock->least_retention_page->physical_stresses;
      currblock->page[j].eqn_cycle = currblock->least_retention_page->eqn_cycle;
      currblock->page[j].stress_increment = currblock->least_retention_page->stress_increment;
      currblock->page[j].retention_period = currblock->least_retention_page->retention_period;
    }    
...
void ssd_verify_parameters(ssd_t *currdisk)
{
    ...
    ASSERT((currdisk->params.planes_per_pkg * currdisk->params.blocks_per_plane) == (unsigned int)currdisk->params.blocks_per_element);
...
void ssd_alloc_queues(ssd_t *t)
{
   ...
   for (i = 0; i < SSD_NUM_GANG(t); i ++) {
          struct ioq *q = (struct ioq*) malloc(sizeof(ioqueue));
   ...
   for (i=0;i<t->params.nelements;i++) {
		...
		//Also allocate one common refresh queue for the ssd..
    ll_create(&t->refresh_queue);
}
