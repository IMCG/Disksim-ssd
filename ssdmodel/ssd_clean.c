...
/*
 * return if a block is dead or not.
 */

inline int ssd_block_dead(block_metadata *block_metadata,ssd_t *s) {
  int is_dead = 0;
  if(block_metadata->state != SSD_BLOCK_DEAD) {//if not set, check the retention period.
    switch(s->params.refresh_policy) {
      case SSD_REFRESH_NONE:
        if (current_retention_period(block_metadata) < s->params.virtual_retention_period)
          is_dead = 1;
      break;
     case SSD_FCFS_REFRESH:
     case SSD_EDF_REFRESH:
        if (current_retention_period(block_metadata) <=s->params.min_retention_period)
          is_dead = 1;
      break;
	 }
    ...
    if (is_dead == 1) {
      ...
      int block_num = block_metadata->block_num;
      s->elements[elem_num].metadata.plane_meta[plane_num].dead_blocks++;
      if (block_metadata->state == SSD_BLOCK_CLEAN) { 
        //reduce free block count only if it was free in the first place.
        s->elements[elem_num].metadata.plane_meta[plane_num].free_blocks--;
        s->elements[elem_num].metadata.tot_free_blocks--;
      }
      int bitpos = ssd_block_to_bitpos(s, block_num);
      assert(s->elements[elem_num].metadata.plane_meta[plane_num].free_blocks>=0 && s->elements[elem_num].metadata.tot_free_blocks >=0);
      ssd_set_bit(s->elements[elem_num].metadata.free_blocks, bitpos); //this block is no longer free!
      fprintf(stderr,"elem #%d plane #%d block #%d is dead. RiP:%.3lf!!\n",
          block_metadata->elem_num,block_metadata->plane_num,block_metadata->block_num,
          block_metadata->least_retention_page->retention_period);
      block_metadata->state = SSD_BLOCK_DEAD;
    //block_metadata->least_retention_page->retention_period = 0;
    }
  }
	return (block_metadata->state == SSD_BLOCK_DEAD);
}

inline int ssd_virtual_retention_expired(block_metadata *block_metadata,ssd_t *s) {
  return (current_retention_period(block_metadata)<=s->params.virtual_retention_period);
/*
  if(block_metadata->state != SSD_BLOCK_RETENTION_LIMIT) {//if not set, check the retention period.
      if(current_retention_period(block_metadata)<s->params.virtual_retention_period)
        block_metadata->state = SSD_BLOCK_RETENTION_LIMIT;
      if(block_metadata->state == SSD_BLOCK_RETENTION_LIMIT) {
        fprintf(stderr,"elem #%d plane #%d block #%d is dead. RiP:%.3lf!!\n",
            block_metadata->elem_num,block_metadata->plane_num,block_metadata->block_num,
            block_metadata->least_retention_page->retention_period);
        //block_metadata->least_retention_page->retention_period = 0;
	...
	return (block_metadata->state == SSD_BLOCK_RETENTION_LIMIT);
*/
}


inline double current_retention_period(block_metadata *block_metadata) {
  ssd_page_metadata *lrp = block_metadata->least_retention_page;
  double time_after_last_stress = (simtime - lrp->time_of_last_stress)/1000;
  if(time_after_last_stress<0)
    time_after_last_stress = 0;
  // KJ: retention cannot be negative in theory
//  assert(lrp->retention_period > 0);
//  if((lrp->retention_period - time_after_last_stress) >= 0)
       return lrp->retention_period - time_after_last_stress;
 // else if(lrp->retention_period > 0)
 //      return lrp->retention_period;
//  else 
  //     return lrp->retention_period - time_after_last_stress;
  //	return 0;
}
...
/*
 * writes a page to the current active page. if there is no active page or if the block is dead,
...
static double ssd_move_page(int lpn, int from_blk, int plane_num, int elem_num, ssd_t *s)
{
    ...
    //check if the active block is dead or not. if dead, allocate a new block and move.
    ...
    switch(s->params.copy_back) {
        case SSD_COPY_BACK_DISABLE:
            if (ssd_last_page_in_block(metadata->active_page,s) || 
                ssd_block_dead(&metadata->block_usage[active_block],s) ) {
                _ssd_alloc_active_block(-1, elem_num, s);
            }
            break;

        case SSD_COPY_BACK_ENABLE:
            if (metadata->plane_meta[plane_num].free_blocks > 0) {
              ASSERT(metadata->plane_meta[plane_num].active_page == metadata->active_page);
            }
            int status = 1;
            if (ssd_last_page_in_block(metadata->active_page, s) ||
                 ssd_block_dead(&metadata->block_usage[active_block],s) ) {
                status = _ssd_alloc_active_block(plane_num, elem_num, s);
                if (status == 0)
                  status = _ssd_alloc_active_block(-1, elem_num, s);
                if (status == 0) {
                  printf("Error: this plane %d in elem %d is full\n", plane_num,elem_num);
...
(int lp_num, int pp_index, int blk, int plane_num, int elem_num, ssd_element_metadata *metadata, ssd_t *s)
{
    ...
    ASSERT(metadata->block_usage[blk].page[pp_index].lpn == -1);
...
void ssd_update_block_lifetime(double time, int blk, ssd_element_metadata *metadata)
{
    ...
    /*if (ssd_block_dead(&metadata->block_usage[blk])) {
...
void ssd_update_free_block_status(int blk, int plane_num, ssd_element_metadata *metadata, ssd_t *s)
{
    ...
    assert(!ssd_block_dead(&metadata->block_usage[blk],s)); //Make sure the selected block is alive.
...
(int blk, int plane_num, int elem_num, ssd_element_metadata *metadata, ssd_t *s)
{
    if (plane_num != -1) {
		...
		if(ssd_block_dead(&metadata->block_usage[blk],s))
			return 0;
...
int ssd_migrate_cold_data(int to_blk, double *mcost, int plane_num, int elem_num, ssd_t *s)
{
    ...
    // erasure time and is alive.
    if (plane_num == -1) {
        for (i = 0; i < s->params.blocks_per_element; i ++) {
            if (metadata->block_usage[i].num_valid > 0 && !ssd_block_dead(&metadata->block_usage[i],s)) {
        ...
        for (i = bitpos; i < bitpos + (int)s->params.blocks_per_plane; i ++) {
            ...
            if (metadata->block_usage[block].num_valid > 0 && !ssd_block_dead(&metadata->block_usage[block],s)) {
    ...
    ASSERT(from_blk != -1);
    assert(!ssd_block_dead(&metadata->block_usage[from_blk],s)); //Make sure the selected block is alive.
    if (plane_num != -1) {
    ...
    // cold data
    cost += _ssd_clean_block_fully(to_blk, metadata->block_usage[to_blk].plane_num, elem_num, metadata, s, is_refresh);
    ...
    //metadata->block_usage[to_blk].total_pages_written += metadata->block_usage[from_blk].num_valid;
    metadata->block_usage[from_blk].num_valid = 0;

    int pages_valid = 0;
    for (i = 0; i < s->params.pages_per_block; i ++) {
        int lpn = metadata->block_usage[from_blk].page[i].lpn;
        if (lpn != -1) {
            ...
            //ssd_update_stress_info(&metadata->block_usage[to_blk].page[i],simtime);
            pages_valid++;
        }
        metadata->block_usage[to_blk].page[i].lpn = metadata->block_usage[from_blk].page[i].lpn;
    }
   // assert(!ssd_block_dead(&metadata->block_usage[to_blk],s)); //Make sure the selected block is alive.
   // assert(pages_valid == metadata->block_usage[to_blk].num_valid );
...
static double _ssd_clean_block_partially(int plane_num, int elem_num, ssd_t *s)
{
    ...
    if (metadata->block_usage[block].num_valid > 0) {
        ...
        for (i = 0; i < s->params.pages_per_block; i ++) {
            int lp_num = metadata->block_usage[block].page[i].lpn;

            if (lp_num != -1) {
       //        if((simtime + cost)  > 1439968085 && (simtime + cost) < 1439968094)
       //        	fprintf(stderr, "Stress caused by cleaning\n");
 ...
 //       if((simtime + cost) > 1439968085 && (simtime + cost) < 1439968094)
    //      fprintf(stderr, "Erase of block %d elem_num %d at time %lf with simtime %lf\n", block, elem_num, simtime+cost, simtime);
        ssd_update_stress_info_block(&metadata->block_usage[block],simtime+cost,s);
...
double _ssd_clean_block_fully(int blk, int plane_num, int elem_num, ssd_element_metadata *metadata, ssd_t *s, int is_refresh)
{
    ...
    if(is_refresh) {
      pm->num_refreshes++;
      s->elements[elem_num].stat.num_refresh ++;  
    }
    else {
      // stat
      pm->num_cleans ++;
...
static double ssd_clean_block_fully(int plane_num, int elem_num, ssd_t *s)
{
    ...
    double cost = 0;
    double mcost = 0;
    int is_refresh = 0;//No, this is not a refreh operation. This is a clean operation.
    ssd_element_metadata *metadata = &s->elements[elem_num].metadata;
    plane_metadata *pm = &metadata->plane_meta[plane_num];
    ...
    cost = _ssd_clean_block_fully(blk, plane_num, elem_num, metadata, s, is_refresh);
    return (cost+mcost);
}
    ...
    do {
        ...
        if (ssd_can_clean_block(s, metadata, blk)) {
            ...
            if (valid_pages == s->params.pages_per_block) {
                ...
                cost += _ssd_clean_block_fully(blk, elem_num, metadata, s, is_refresh);
    ...
    for (i = 0; i <= s->params.pages_per_block; i ++) {
        ...
        for (j = 0; j < entry->len; j ++) {
            ...
            if (ssd_can_clean_block(s, metadata, blk)) {
                ...
                // invoke cleaning until we reach the high watermark.
                cost += _ssd_clean_block_fully(blk, metadata->block_usage[blk].plane_num, elem_num, metadata, s,is_refresh);
...
double ssd_refresh_element_no_copyback(int elem_num, ssd_t *s, block_metadata *bm)
{
    double cost = 0;
    ASSERT(0);
    return cost;
...
inline int ssd_next_plane_in_parunit(int plane_num, int parunit_num, int elem_num, ssd_t *s)
{
